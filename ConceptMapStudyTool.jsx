...
// ─── Data ────────────────────────────────────────────────────────────────────
const NODES = [
  { id: "clt", label: "Cognitive Load\nTheory", def: "A theory by John Sweller (1988) proposing that learning is constrained by the limited capacity of working memory. Effective instruction design minimizes unnecessary load while maximizing schema formation.", source: "Slide 2 · Sweller (1988) — foundational overview, three-load taxonomy", x: 420, y: 260 },
...
function computeStudyPath(tags) {
  ...
  // For each needs_work node, add its shaky/needs_work prereqs first
  needsWork.forEach(nw => {
    const prereqs = getPrereqs(nw);
    prereqs.forEach(p => {
      if (!added.has(p) && tags[p] && tags[p] !== "clear") {
        path.push({ id: p, reason: `Prerequisite for ${getNodeById(nw)?.label.replace("\n", " ")}` });
        added.add(p);
      }
    });
    if (!added.has(nw)) {
  ...
  // Then add remaining shaky nodes
  shaky.forEach(s => {
    if (!added.has(s)) {
...
function edgePath(from, to, idx) {
  ...
  return { path: `M ${from.x} ${from.y} Q ${cx} ${cy} ${to.x} ${to.y}`, cx, cy, mx, my };
}

// ─── Components ──────────────────────────────────────────────────────────────

function ProgressBar({ tags }) {
  ...
  const untagged = 12 - clear - shaky - needsWork;
  const total = 12;

  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, width: "100%" }}>
      <div style={{ flex: 1, height: 10, borderRadius: 5, overflow: "hidden", display: "flex", background: "#E5E5E5" }}>
        {clear > 0 && <div style={{ width: `${(clear / total) * 100}%`, background: "linear-gradient(90deg, #34D399, #6EE7B7)", transition: "width 0.5s ease" }} />}
        {shaky > 0 && <div style={{ width: `${(shaky / total) * 100}%`, background: "linear-gradient(90deg, #FBBF24, #FCD34D)", transition: "width 0.5s ease" }} />}
        {needsWork > 0 && <div style={{ width: `${(needsWork / total) * 100}%`, background: "linear-gradient(90deg, #F87171, #FCA5A5)", transition: "width 0.5s ease" }} />}
      </div>
      <div style={{ display: "flex", gap: 12, fontSize: 12, fontFamily: "'DM Sans', sans-serif", color: "#6B7280", whiteSpace: "nowrap", letterSpacing: "0.01em" }}>
...
function StudyPath({ path, onNodeClick, expanded, onToggle }) {
  if (!path) return null;
  return (
    <div style={{
      ...
      <div onClick={onToggle} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 13, color: "#92400E" }}>
          ...
          {path.map((item, i) => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,1)"; e.currentTarget.style.borderColor = "#FBBF24"; }}
...
function SidePanel({ node, tags, onTag, onClose, onNavigate }) {
  if (!node) return null;
  const tag = tags[node.id];
  const related = [];
  EDGES.forEach(e => {
    if (e.from === node.id && !related.find(r => r.id === e.to)) related.push({ id: e.to, label: getNodeById(e.to)?.label.replace("\n", " "), rel: e.label });
    if (e.to === node.id && !related.find(r => r.id === e.from)) related.push({ id: e.from, label: getNodeById(e.from)?.label.replace("\n", " "), rel: e.label });
  });
  ...
  return (
    <div style={{
      ...
      <button onClick={onClose} style={{
        ...
        border: "1px solid #E5E7EB", background: "white", cursor: "pointer", fontSize: 14, color: "#9CA3AF",
...
export default function ConceptMapApp() {
  ...
  // Pulse animation for needs_work nodes
  useEffect(() => {
    const interval = setInterval(() => setPulsePhase(p => (p + 1) % 60), 50);
    return () => clearInterval(interval);
  }, []);

  const connected = useMemo(() => selected ? getConnected(selected) : { nodes: new Set(), edges: new Set() }, [selected]);
  const studyPath = useMemo(() => computeStudyPath(tags), [tags]);
  ...
  const hoveredNode = hovered ? getNodeById(hovered) : null;
  ...
  const pulseVal = Math.sin(pulsePhase * 0.1) * 0.5 + 0.5; // 0..1
  ...
  return (
    <div style={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column", background: bg, fontFamily: "'DM Sans', sans-serif", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        {settingsOpen && (
          <div style={{
            display: "flex", gap: 20, padding: "8px 0", fontSize: 12, color: textSecondary,
          }}>
        ...
        {/* Study Path */}
        <StudyPath path={studyPath} onNodeClick={handleSelectNode} expanded={pathExpanded} onToggle={() => setPathExpanded(!pathExpanded)} />
      </div>
        ...
        <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
          ...
          >
            ...
            {EDGES.map((edge, i) => {
              ...
              const { path, cx, cy } = edgePath(from, to, i);
            ...
            {NODES.map(node => {
              ...
              if (tag === "clear") {
                fill = darkMode ? "#1A2E1A" : "#ECFDF5";
                stroke = "#34D399";
                strokeW = 2;
              } else if (tag === "shaky") {
                fill = darkMode ? "#2E2A1A" : "#FFFBEB";
                stroke = "#FBBF24";
                strokeW = 2;
              } else if (isNeedsWork) {
              ...
              const w = node.id === "clt" ? 140 : (node.label.includes("\n") ? 120 : 100);
              ...
              return (
                ...
                >
                  ...
                  {node.label.split("\n").map((line, li) => (
                    <text
                      key={li}
                      x={node.x} y={node.y + (li - (node.label.split("\n").length - 1) / 2) * 15 + 1}
                      textAnchor="middle" dominantBaseline="middle"
                      style={{
                        fontSize: node.id === "clt" ? 12 : 11,
                        fontWeight: node.id === "clt" ? 700 : 600,
